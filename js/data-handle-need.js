...
function buildSelect3(name, data, childs) {
	...
	var select = $('<input list="reg-searching-need-'+data.name +'3" id="'+data.name+"-"+name+'3">');
	select.addClass('form-control');
	var dlist = $('<datalist id="reg-searching-need-'+data.name +'3">');
    var option = $('<option>');
    dlist.append(option);
...
var showTableNeed = function () {
	...
	var item;
	if (region.length>=2){
		needhelp.doc(country).collection(region).get().then(function(querySnapshot) {
			...
			var $t_table = $('#table-need');
			$('#need-placeholder').addClass('hidden');
		...
		d.childs.forEach(function (values) {
			needhelp.doc(country).collection(values.name).get().then(function(querySnapshot) {
				querySnapshot.forEach(function(doc) {
					item = doc.data();
					var options = {
						day:'numeric', month:'numeric', year:'2-digit', hour: "2-digit", minute:'2-digit'
...
jQuery(document).ready(function($) {
	$('#search-need').on('click', showTableNeed);
	buildForms(data);
