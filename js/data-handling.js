var firebaseConfig = {
 ...
 var data = {
  ...
  function buildSelect(name, data, childs) {
	...
	var select = $('<input list="searching-need-'+data.name +'" id="'+data.name+"-"+name+'">');
	select.addClass('form-control');
	var dlist = $('<datalist id="searching-need-'+data.name+'">');
    var option = $('<option>');
    dlist.append(option);
...
function buildSelect2(name, data, childs) {
	...
	label.text(name);
	label.addClass('text-white mt-2 mb-1 text-uppercase');
	var select = $('<input list="searching-give-'+data.name +'2" id="'+data.name+"-"+name+'2">');
	select.addClass('form-control');
	var dlist = $('<datalist id="searching-give-'+data.name+'2">');
    var option = $('<option>');
    dlist.append(option);
    data.childs.forEach(function (child) {
       option = $('<option>');
